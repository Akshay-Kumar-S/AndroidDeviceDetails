package com.example.androidDeviceDetails.models.signal

object Signal {
    const val CELLULAR=0
    const val WIFI=1
    const val CELLULAR_MIN = -120
    const val CELLULAR_RANGE = 96
    const val WIFI_MIN = -120
    const val WIFI_LEVEL = 45
    const val WIFI_RANGE = 97
    const val GRAPH_PLOT_POINTS = 40
    const val LEVEL_POOR = 0
    const val LEVEL_LOW = 1
    const val LEVEL_MEDIUM = 2
    const val LEVEL_GOOD = 3
    const val LEVEL_EXCELLENT = 4
    const val FOURTH_GEN="4g"
    const val THIRD_GEN="3g"
    const val SECOND_GEN="2g"
    const val FIFTH_GEN="5g"
    const val TYPE_LTE="LTE"
    const val TYPE_GSM="GSM"
    const val TYPE_CDMA="CDMA"
    const val TYPE_NR="NR"
    const val TYPE_WCDMA="WCDMA"
    const val TYPE_TDSCDMA="TDSCDMA"
    const val NO_DATA="Unknown"
}